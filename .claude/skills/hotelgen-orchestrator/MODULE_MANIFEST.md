...
| A7 | **H1** wrap whole webhook POST body in try/catch → always 200 (stop Telegram retry storm on throw) | 🔧 |
| A8 | **H3** `verification_attempts` insert: `await` + error check (don't lose security log) | 🔧 |
| A9 | **H4** in-house link `update`s: check `error`, else infinite "ask room number" loop | 🔧 |
...
| A12 | **D4** extend archive cron to `inhouse_guests_v2` (`status active→archived`, `check_out_date<today`) — else persistent-verify treats checked-out as active | 🔧 |
