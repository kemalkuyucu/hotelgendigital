...
| A10 | **H2** + finding #1 — double escalation | ✅ `v1.0-A10-sla-atomic-claim` (commit `f33c6b9`) 2026-06-01 — H2: `getBotTokenForHotel` demo→env / else `getDecryptedBridge(id).telegramBotToken`. #1 (double escalation): **atomic claim** — `update(escalated_at) WHERE escalated_at IS NULL .select()` before sending; skip if not claimed (DB row-lock + re-eval WHERE → single message on concurrent calls). External dup cron (cron-job.org) removed by Kemal. **POST-DEPLOY REAL-BOT VERIFIED ✅ 2026-06-01** — doğrulanmış misafir (102) talep gönderdi, butona basılmadı, SLA aşımında Demo_OnBuro'ya eskalasyon TAM 1 KEZ düştü (önceden 2 kez), "Oda: 102" dolu. **Finding #3 reclassified → AUDIT D5 = A11** |
...
## Phase B — BRAIN RE-ARCHITECTURE — 🚧 BAŞLADI (B2.1+B2.2 ✅ 2026-06-01) · SIRADAKİ B2.3
...
> **Tip→davranış:** SOHBET/BİLGİ forward yok · TALEP=forward+2 buton+sla_events · BİLDİRİM=forward+buton YOK+sla_events YOK.
...
| B2.2 | `routeIntentToDepartment` → `messageType`+`withButtons`/`createsSlaEvent` flag'leri döndür; B2.1 haritasını tüket (çoğullamayı gider). Davranış-nötr. type-check+build | ✅ `v1.0-B2.2-router-consumes-map` 2026-06-01 — TEK dosya `classify-and-respond.ts` (+47/−13). `RoutingDecision`+`ClassifiedIntentItem` interface'lerine `messageType`/`withButtons`/`createsSlaEvent` eklendi; `routeIntentToDepartment` her 5 dönüşe `...typeSignature` spread eder (departman + `shouldForward` branch'leri AYNEN korundu — additive); `classifiedIntents.map` flag'leri her intent'e taşır (çoklu-intent: TALEP+BİLDİRİM ayrışır). `NON_FORWARDING_INTENTS` elle-kopya liste → `new Set([...CHAT_INTENTS, ...INFO_INTENTS])` (aynı 7 üye, export korundu, çoğullama gitti). **route.ts DOKUNULMADI** — allergy hâlâ `rawDepartment==='allergy'` ile dallanır, yeni flag'ler B2.3'e dek TÜKETİLMEZ. **Davranış-nötr doğrulandı** (single+multi-intent çıktısı bugünküyle birebir). type-check+build YEŞİL, bot testi yok. SIRADAKİ → B2.3 |
...
| B1.1 | Constitution dosyaları `00-master` + `01-front-office`…`07-animation` (saf içerik, wire yok) | 🔧 |
| B1.2 | Lazy-load loader (departmana göre constitution → prompt kompozisyonu), **flag arkasında no-op** (davranış değişmez). type-check+build | 🔧 |
| B1.3 | Orchestrator swap: `system-prompts.ts` monolit → 00-master + ilgili dept constitution. **GENİŞ bot testi** — YÜKSEK RİSK | 🔧 |
| B4 | Regresyon: persistent verify + forward B1.3 sonrası hâlâ yeşil (gate) | 🔶 |
