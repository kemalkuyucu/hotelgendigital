...
export default function SpaUploadPage() {
  ...
  async function handleFile(f: File) {
    ...
    try {
      ...
      fd.append('file', f);
      const res = await fetch(`/api/hotel-admin/${slug}/spa/parse-excel`, { method: 'POST', body: fd });
  ...
  async function handleImport() {
    ...
    try {
      ...
      const res = await fetch(`/api/hotel-admin/${slug}/spa/import`, {
        method: 'POST',
  ...
  return (
    <div style={{ maxWidth: 820, margin: '0 auto', padding: '40px 24px', width: '100%' }}>
      ...
      <p style={{ color: '#94a3b8', marginBottom: 28, fontSize: 14.5, lineHeight: 1.6, maxWidth: 640 }}>
        Hizmet listenizi Excel ile yükleyin; bot anında yeni listeyi okumaya başlar. Yeni yükleme,
        mevcut listenin tamamının yerini alır. Açıklama sütununa terapist seçenekleri, aroma yağlar,
        faydalar ve uyarıları yazabilirsiniz.
      </p>

      {/* Kolon sablonu rozetleri */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 28 }}>
        {['Hizmet Adi', 'Kategori', 'Sure', 'Fiyat', 'Para Birimi', 'Ucretli mi', 'Aciklama'].map((c) => (
          <span
            key={c}
            style={{
              fontSize: 12.5,
      ...
      {step === 'done' && (
        <div style={{ ...card, padding: 36, textAlign: 'center', borderColor: 'rgba(52,211,153,0.3)' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: 14.5, margin: '0 0 22px' }}>
            <b style={{ color: '#34d399' }}>{resultCount}</b> hizmet kaydedildi. Bot artık yeni listeyi okuyor.
          </p>
