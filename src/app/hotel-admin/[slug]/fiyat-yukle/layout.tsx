...
export default async function FiyatYukleLayout({
  children,
  params,
}: {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0a0f1e 0%, #111827 50%, #0a0f1e 100%)', position: 'relative', overflow: 'hidden' }}>
      <HotelAdminParticleWrapper />
      <div style={{ position: 'relative', zIndex: 10, minHeight: '100vh', display: 'flex' }}>
        <DashboardLayoutClient slug={slug} adminName={admin.full_name} adminRole={admin.role}>
          {children}
